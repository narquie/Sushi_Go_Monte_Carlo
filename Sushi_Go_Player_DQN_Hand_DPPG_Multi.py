...
import Sushi_Go_Player_DPPG_Multi_Agent as Agent
import numpy as np
...
agent = Agent.Agent(state_size=106, action_size=13, random_seed=0)
...
def bot_action(new_game,state):
    ...
    for index, i in enumerate(new_game.players_list[1:]):
        next_state = []
        for j in i.report_cards_in_hand():
            next_state.append(j)
        next_state = np.append(next_state,state)
        legal_actions_bot = i.legal_actions()
        next_state[len(next_state)-2] = i.score
        action = agent.act(next_state,legal_actions_bot)
        # Chopsticks action
        if action == 13:
            legal_actions_bot.remove(action)
            action1 = agent.act(next_state,legal_actions_bot)
            count = 0
            ...
            action2 = agent.act(next_state,legal_actions_bot)
